...
`);
  ...
  async function get_Songs(folder) {
    try {
      currFolder = folder;
      let response = await fetch(`https://rrrinav.github.io/Music-Player/Songs/${folder}/`);
    ...
    try {
      ...
      playlists.forEach(async (playlist) => {
        ...
        let response = await fetch(
          `https://rrrinav.github.io/Music-Player/Songs/${folder}/info.json`
        );
        let info = await response.json();

        cardContainer.innerHTML += `<div class="card-wrapper" data-folder="${folder}">
                    <div class="card p-1">
                        <img src="https://rrrinav.github.io/Music-Player/Songs/${folder}/cover.jpg" alt="">
                        <h4>${info.title}</h4>
  ...
  async function main() {
    ...
    document.querySelector("#vol-range").addEventListener("input", (e) => {
      const volumePercentage = e.target.value;
